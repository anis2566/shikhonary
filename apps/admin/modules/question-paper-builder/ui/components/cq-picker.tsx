...
interface CqPickerProps {
  paperId: string;
  assignedCqIds: string[];
}

export const CqPicker: React.FC<CqPickerProps> = ({
  paperId,
  assignedCqIds,
}) => {
  ...
  const trpc = useTRPC();
  const { data: cqs, isLoading } = useQuery({
    ...
    select: (data: Record<string, unknown>) =>
      (data as Record<string, unknown>).data as
        | { items: CQ[]; totalCount: number }
        | undefined,
  });

  const { mutateAsync: assignCq } = useAssignCqToQuestionPaper();

  const handleAssign = async (cqId: string) => {
    setAssigningId(cqId);
    try {
      await assignCq({
  ...
  const filteredItems =
    (cqs as { items: CQ[]; totalCount: number } | undefined)?.items || [];
  const totalCount =
    (cqs as { items: CQ[]; totalCount: number } | undefined)?.totalCount ??
    ...
    <div className="flex flex-col h-full bg-background border-l">
      ...
      <div className="p-4 border-b space-y-3">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="font-bold text-lg">CQ Question Bank</h3>
        ...
        <div className="p-4 space-y-3">
          {isLoading ? (
            ...
            <div className="text-center py-12 text-muted-foreground">
              <HelpCircle className="mx-auto h-8 w-8 opacity-20 mb-2" />
              <p className="text-sm font-medium">No CQs found</p>
              {activeFilterCount > 0 && (
                <Button
                  variant="link"
                  size="sm"
            ...
            filteredItems.map((cq: CQ) => {
              const isAssigned = assignedCqIds.includes(cq.id);
              const isThisAssigning = assigningId === cq.id;
              ...
              return (
                ...
                >
                  <div className="space-y-2">
                    <p className="text-xs font-medium text-muted-foreground line-clamp-2 italic mb-1">
                        {cq.context}
                    </p>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      ...
                      >
                        ...
                        {cq.subject?.displayName || "Subject"}
                      </Badge>
                      <Badge
                        variant="outline"
                        className="text-[10px] uppercase font-black px-2 py-0 h-5"
                      >
                        ...
                        {cq.chapter?.displayName || "Chapter"}
                      </Badge>
                    </div>
                  </div>
