...
export function Filters({ viewMode, onViewModeChange }: FiltersProps) {
  ...
  const { data: batches, isLoading: isBatchesLoading } = useBatchByYearClassId(
      ...
      <div className="bg-white p-4 flex flex-wrap items-center gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Select
            ...
            disabled={isBatchesLoading || batch_options.length === 0}
          >
            <SelectTrigger
              disabled={isBatchesLoading || batch_options.length === 0}
              className="bg-[#eff4ff] border-none rounded-[12px] text-sm font-semibold text-[#0b1c30] w-[150px] h-10 px-4 focus:ring-2 focus:ring-primary/20 transition-all hover:bg-[#e5eeff] disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <SelectValue
                placeholder={isBatchesLoading ? "Loading..." : "All Batches"}
              />
            </SelectTrigger>
            <SelectContent className="rounded-xl border-slate-100 shadow-ambient bg-white/95 backdrop-blur-md">
            ...
            <div className="flex flex-wrap items-center gap-2 px-4 pb-4 pt-0">
              ...
              {filters.classId && (
                ...
                >
                  ...
                  <span className="font-bold text-[11px]">
                    {
                      class_options.find(
                        (item) => item.value === filters.classId,
                      )?.label
                    }
                  </span>
                  <button
              ...
              {filters.batchId && (
                ...
                >
                  ...
                  <button
                    onClick={() =>
                      setFilters({ ...filters, batchId: null })
                    }
                    className="hover:text-rose-500 transition-colors ml-1"
                  >
                    <X className="w-3 h-3 stroke-[3]" />
