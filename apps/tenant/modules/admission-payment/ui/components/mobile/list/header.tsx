...
export const Header = () => {
  ...
  const [search, setSearch] = useState(filters.studentSearch || "");
  const debounceValue = useDebounce(search, 500);

  useEffect(() => {
    setFilters({ ...filters, studentSearch: debounceValue || null });
  }, [debounceValue]);

  // Sync internal search state when filters change externally (e.g., reset)
  useEffect(() => {
    if (filters.studentSearch !== search) {
      setSearch(filters.studentSearch || "");
    }
      ...
      <div className="flex items-center justify-between px-4 pt-4 pb-3">
        ...
        <div className="flex items-center gap-2.5 min-w-0">
          <div className="relative flex-shrink-0">
            <div className="w-9 h-9 rounded-[10px] bg-emerald-500 flex items-center justify-center shadow-md shadow-emerald-500/30">
        ...
        <Button
          ...
          "
        >
          <Link href="/payments/admission-payments/new">
