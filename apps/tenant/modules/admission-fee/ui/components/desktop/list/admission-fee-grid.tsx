...
import { AdmissionFeeCard } from "./admission-fee-card";
import { Skeleton } from "@workspace/ui/components/skeleton";

interface AdmissionFeeWithRelations {
  id: string;
  amount: number;
  className: string;
...
interface AdmissionFeeGridProps {
  admissionFees: AdmissionFeeWithRelations[];
  isLoading: boolean;
  onDelete: (id: string, name: string) => void;
}

export function AdmissionFeeGrid({
  admissionFees,
  isLoading,
  onDelete,
}: AdmissionFeeGridProps) {
  if (isLoading) {
    return (
      ...
      <div className="py-20 flex flex-col items-center justify-center text-center space-y-4">
        ...
        <div className="space-y-1">
          ...
          <p className="text-slate-500 max-w-xs mx-auto text-sm leading-relaxed">
            There are no admission fees matching your criteria. Try adjusting your
